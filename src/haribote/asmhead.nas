...
DSKCAC	EQU		0x00100000		; 内存中保存软盘内容的起始地址
DSKCAC0	EQU		0x00008000		; 软盘读取的起始地址

; BOOT_INFO 启动信息
CYLS	EQU		0x0ff0			; 设定启动区
LEDS	EQU		0x0ff1
VMODE	EQU		0x0ff2			; 有关验收数目的信息，颜色的位数
...
; 画面模式的切换
		...
		MOV		AX,0x4f02
		INT		0x10
		MOV		BYTE [VMODE],8	; 记录下画面模式（参考C语言）
		MOV		AX,[ES:DI+0x12]
		MOV		[SCRNX],AX
		MOV		AX,[ES:DI+0x14]
		MOV		[SCRNY],AX
...
scrn320:
		MOV		AL,0x13			; VGA图、320x200 8bit彩色
		MOV		AH,0x00
		INT		0x10
		MOV		BYTE [VMODE],8	; 记录下画面模式（参考C语言）
		MOV		WORD [SCRNX],320
